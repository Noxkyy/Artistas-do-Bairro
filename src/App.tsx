...
export default function App() {
  ...
  useEffect(() => { window.localStorage.setItem(FAV_KEY, JSON.stringify(favorites)); }, [favorites]);
  ...
  return (
    <div className="relative min-h-screen bg-bg text-ink">
      ...
      <main>
        ...
        <Stats />
        <Categories active={category} onSelect={(id) => setCategory(id)} />
...
function ScrollProgress() {
  ...
  useEffect(() => {
    let raf = 0;
    const onScroll = () => { cancelAnimationFrame(raf); raf = requestAnimationFrame(() => { const h = document.documentElement.scrollHeight - window.innerHeight; setPct(h > 0 ? (window.scrollY / h) * 100 : 0); }); };
  ...
  return <div className="fixed inset-x-0 top-0 z-50 h-[3px] bg-transparent" aria-hidden="true"><div className="h-full bg-accent shadow-[0_0_12px_var(--c-glow)] transition-[width] duration-150 ease-out" style={{ width: `${pct}%` }} /></div>;
}
